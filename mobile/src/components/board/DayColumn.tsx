...
import { Plus, Target, FileText, Hand } from 'lucide-react-native';
...
export const DayColumn: React.FC<DayColumnProps> = ({
  ...
  const handleDropTask = useCallback(
    async (taskId: string) => {
      ...
      if (!task || task.date === day.dateString) return;
      try {
        await updateTask(taskId, { date: day.dateString });
        showToast(
          `Tarea "${task.title}" movida a ${day.name} (${day.dateString})`,
          'success',
          '// DÍA REASIGNADO'
        );
      } catch (e) {
        console.error('Error moving task on drop:', e);
      }
    },
    [allTasks, day.dateString, day.name, updateTask, showToast]
  );
      ...
      style={[
        styles.columnContainer,
        {
          ...
          borderColor: isDragOver
            ...
            ? colors.accent
            : colors.borderColor,
          borderWidth: isDragOver ? 2.5 : THEME.borders.thick,
        },
        isMobile ? styles.columnMobile : isDesktop ? styles.columnDesktop : { width: columnWidth },
      ]}
      // @ts-ignore
      dataSet={{ role: 'day-column', date: day.dateString }}
      {...(Platform.OS === 'web'
        ? {
            onDragOver: (e: any) => {
              e.preventDefault();
    ...
    >
      ...
      >
        {isDragOver ? (
          ...
          >
            <Hand size={13} color={colors.accent} strokeWidth={2.5} />
            <Text style={[styles.dragOverText, { color: colors.accent }]}>
              SOLTAR PARA MOVER A {day.name.toUpperCase()}
            </Text>
          </View>
        ) : null}
