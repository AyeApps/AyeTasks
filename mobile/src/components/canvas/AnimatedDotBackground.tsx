...
const WebInteractiveDots: React.FC<{ isDark: boolean }> = ({ isDark }) => {
  ...
  useEffect(() => {
    ...
    let currentThemeFactor = isDarkRef.current ? 1 : 0;
    ...
    const render = (now: number) => {
      ...
      // Smooth 350ms synchronized transition between dark (1) and light (0)
      const targetThemeFactor = isDarkRef.current ? 1 : 0;
      currentThemeFactor += (targetThemeFactor - currentThemeFactor) * 0.10;
      if (Math.abs(targetThemeFactor - currentThemeFactor) < 0.001) {
        currentThemeFactor = targetThemeFactor;
      }
      const t = currentThemeFactor;

      // Cyber-Amber accent colors (smoothly interpolated)
      const amberR = Math.round(230 + (254 - 230) * t);
      const amberG = Math.round(138 + (157 - 138) * t);
      const amberB = Math.round(0 + (1 - 0) * t);

      // Neutral dot colors (smoothly interpolated)
      const baseAlpha = 0.075 + (0.085 - 0.075) * t;
      const baseR = Math.round(0 + 255 * t);
      const baseG = Math.round(0 + 255 * t);
      const baseB = Math.round(0 + 255 * t);
